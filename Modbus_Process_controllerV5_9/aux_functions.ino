...
//pin definitions
  ...
  platform.hornPin = 4;
  platform.relayPin = 3;
  platform.pulseInPin = 0;
  platform.buttonInPin = 9;
  platform.analogPin = 1;
  platform.SDApin = 5;  //INTERNAL
  platform.SCLpin = 6;  //INTERNAL
  if (legacy_platform_mode == 1) platform.pulseInPin = 2;
#endif
  ...
  platform.hornPin = 4;
  platform.relayPin = 0;
  platform.pulseInPin = 18;
  platform.buttonInPin = 5;
  platform.analogPin = 35;
  platform.SDApin = -1;  // Use -1 for the Wire library default pins
  platform.SCLpin = -1;  // Use -1 for the Wire library default pins
#endif

#ifdef S2_mini
  platform.ledOn = 1;
  platform.ledPin = 15;  // ESP32-S2 lolin s2 mini
  platform.hornPin = 7;
  platform.relayPin = 5;
  platform.pulseInPin = 3;
  platform.buttonInPin = 0;
  platform.analogPin = 1;
  platform.SDApin = 33;  // Use -1 for the Wire library default pins
  platform.SCLpin = 35;  // Use -1 for the Wire library default pins
#endif

  pinMode(platform.ledPin, OUTPUT);
  pinMode(platform.hornPin, OUTPUT);
  pinMode(platform.relayPin, OUTPUT);
  pinMode(platform.pulseInPin, INPUT_PULLUP);
  pinMode(platform.buttonInPin, INPUT_PULLUP);
...
void configure_interrupt_type() {
  ...
  if (pin_mode == 0) {
    attachInterrupt(digitalPinToInterrupt(platform.pulseInPin), pin_interrupt, RISING);
  }
  if (pin_mode == 1) {
    attachInterrupt(digitalPinToInterrupt(platform.pulseInPin), pin_interrupt, FALLING);
  }
  if (pin_mode == 2) {
    attachInterrupt(digitalPinToInterrupt(platform.pulseInPin), pin_interrupt, CHANGE);
  }
  if (pin_mode > 2) {
    detachInterrupt(digitalPinToInterrupt(platform.pulseInPin));  // Disable the interrupt
  }
}
...
int senseButton() {
  ...
  for (int t = 0; t < 11; t++) {
    if (digitalRead(platform.buttonInPin) == 0) {
      debounce++;
    }
  }
...
void setRelay(uint16_t newState) {
  ...
  digitalWrite(platform.relayPin, pinstate);
...
void setHorn(uint16_t newState) {
  ...
  digitalWrite(platform.hornPin, pinstate);
...
uint16_t getAnalog() {
  ...
  long analogAvg = 0;
  analogSetPinAttenuation(platform.analogPin, ADC_11db);  // set ADC range to 2.6v for more sensitivity
  for (int t = 0; t < 10; t++) {
    analogAvg = analogAvg + analogRead(platform.analogPin);
  }
...
void dataPage() {
  ...
  Serial.print("\nHorn:");
  Serial.print(digitalRead(platform.hornPin));
  Serial.print("  Relay:");
  Serial.print(digitalRead(platform.relayPin));
  Serial.print("  Flow:");
...
void manPage() {
  ...
  Serial.print("\nLegacy / alt pinouts    <100>-   set to 1 for legacy pinout or ? for other pin specs");
