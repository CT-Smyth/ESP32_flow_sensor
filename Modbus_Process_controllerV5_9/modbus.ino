...
void syncModbusSettings() {
  ...
  mBus.Hreg(LEGACY_PLATFORM_MODE, legacy_platform_mode);
  mBus.task();
}
...
void setupModbus() {
  ...
  mBus.addHreg(RESET_HREG, 0);
  mBus.onSetHreg(RESET_HREG, set_RESET_callback);
  mBus.onGetHreg(RESET_HREG, get_RESET_callback);

  mBus.addHreg(LEGACY_PLATFORM_MODE, 0);
  mBus.onSetHreg(LEGACY_PLATFORM_MODE, set_LEGACY_PLATFORM_MODE_callback);
  mBus.onGetHreg(LEGACY_PLATFORM_MODE, get_LEGACY_PLATFORM_MODE_callback);
}
...
uint16_t set_LEGACY_PLATFORM_MODE_callback(TRegister* reg, uint16_t val) {
  legacy_platform_mode = val;
  saveSettings();
  setPlatform();
  return val;
}
uint16_t get_LEGACY_PLATFORM_MODE_callback(TRegister* reg, uint16_t val) {
  //Serial.print("value = ");
  //Serial.println(val);
  return val;
}
