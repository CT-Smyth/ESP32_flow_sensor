...
#define OLED_WIDTH 128 //72x40 displays in center of 128x64 field 
...
typedef struct {
  ...
  byte buttonInPin;
  byte analogPin;
  byte SDApin;  // Use -1 for the Wire library default pins
  byte SCLpin;  // Use -1 for the Wire library default pins
} pinmap;
...
const int LEGACY_PLATFORM_MODE = 100;
...
uint16_t legacy_platform_mode = 0;
...
uint16_t getInputs(u_long millis_now) {
  ...
  pulse_pin_state = digitalRead(platform.pulseInPin);
...
void loop() {
  ...
  if ((blink > millis()) || ((digitalRead(platform.pulseInPin) != 0) && (active_flow != 0))) {
    digitalWrite(platform.ledPin, platform.ledOn);
  } else {
    digitalWrite(platform.ledPin, !platform.ledOn);
  }
