...
const int SHOW_DISPLAY = 3;  //set value to show display for n minutes (max 120)
...
uint16_t getInputs(u_long millis_now) {
  ...
  if (pin_mode < 3) {  // if input is set to interrupts
    raw_pulses = ISR_sensor_pulses;
    ISR_sensor_pulses = 0;
    return raw_pulses;
  }

  if (pin_mode == 3) {  // if input is set ON = active
  ...
  if (pin_mode == 4) {  // if input is set OFF = active
  ...
  if (pin_mode == 5) {  // if input is set ANALOG
  ...
  if (pin_mode == 6) {  // if input is set ANALOG - Active range - pulse by time
    uint16_t analogAvg = analog_sense;

    if (analogAvg < analog_threshold) {  //start cycle
      cycle_state = 1;
    }
    if (analogAvg > analog_range_max) {  //end cycle
      cycle_state = 0;
    }
  ...
  if (pin_mode == 7) {  // if input is set ANALOG - Active range - analog value
    uint16_t analog_data = analog_sense;
...
// mode 0 is trigger on failure to achieve x_units in y_seconds while flowing (low flow) -- if flow_limit != 0: only valid if (this_flow_rate > flow_limit)
...
// mode 5 is count only. Horn and relay outputs are set by modbus only.
...
uint16_t processActions(uint16_t units_to_process, u_long millis_now) {
  ...
  if (mode == 5) {  //just count - no control
    units_to_process = 0;
  }
...
void loop() {
  ...
  // if ((blink > millis()) || (flashTime > millis())) {
